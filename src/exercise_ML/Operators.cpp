...
for (std::size_t is = 0; is < particles.size(); is++) {
    ...
    // alias
    Particles particle = particles[is];
 
    // position
    Particles::view_t Px = particle.x_m;
    Particles::view_t Py = particle.y_m;
    Particles::view_t Pz = particle.z_m;
    // electric field
    Particles::view_t P_Ex = particle.Ex_m;
    Particles::view_t P_Ey = particle.Ey_m;
    Particles::view_t P_Ez = particle.Ez_m;
    // magnetic field
    Particles::view_t P_Bx = particle.Bx_m;
    Particles::view_t P_By = particle.By_m;
    Particles::view_t P_Bz = particle.Bz_m;

//   for (std::size_t part = 0; part < n_particles; ++part) {
    Kokkos::parallel_for ( "loop on part", n_particles,
	KOKKOS_LAMBDA (const int part) {
	  
      // Calculate normalized positions
      const double ixn = Px(part) * em.inv_dx_m;
      const double iyn = Py(part) * em.inv_dy_m;
      const double izn = Pz(part) * em.inv_dz_m;
      ...
      // Ex (d, p, p)
      {
        ...
        // particles[is].Ex_h_m(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
         P_Ex(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
      }
      ...
      // Ey (p, d, p)
      {
        ...
        // particles[is].Ey_h_m(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
         P_Ey(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
      }
      ...
      // Ez (p, p, d)
      {
        ...
        // particles[is].Ez_h_m(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
         P_Ez(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
      }
      ...
      // Bx (p, d, d)
      {
       ...
       // particles[is].Bx_h_m(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
        P_Bx(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
      }
      ...
      // By (d, p, d)
      {
       ...
       // particles[is].By_h_m(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
        P_By(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
     }
      ...
      // particles[is].Bz_h_m(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
        P_Bz(part) = v0 * (1 - coeffs[2]) + v1 * coeffs[2];
     }
    } // End for each particle

   ); // End of parallel_for

   Kokkos::fence ( "Fence: loop on part" );
...
void push(std::vector<Particles> &particles, double dt) {
  ...
  for (std::size_t is = 0; is < particles.size(); is++) {
    ...
    // q' = dt * (q/2m)
    const double qp = particle.charge_m * dt * 0.5 / particle.mass_m;
    ...
    Particles::view_t P_Bz = particle.Bz_m;
    // mx
    Particles::view_t P_mx = particle.mx_m;
    Particles::view_t P_my = particle.my_m;
    Particles::view_t P_mz = particle.mz_m;

    // for (std::size_t ip = 0; ip < n_particles; ++ip) {
    Kokkos::parallel_for ( "loop on ip", n_particles,
	KOKKOS_LAMBDA (const int ip) {

    // 1/2 E
      double px = qp * P_Ex(ip);
      double py = qp * P_Ey(ip);
      double pz = qp * P_Ez(ip);
      
      const double ux = P_mx(ip) + px;
      const double uy = P_my(ip) + py;
      const double uz = P_mz(ip) + pz;
      ...
      const double tx = gamma_inv * P_Bx(ip);
      const double ty = gamma_inv * P_By(ip);
      const double tz = gamma_inv * P_Bz(ip);
      ...
      // Update positions
      Px(ip) += P_mx(ip) * dt * gamma_inv;
      Py(ip) += P_my(ip) * dt * gamma_inv;
      Pz(ip) += P_mz(ip) * dt * gamma_inv;
 
    }

   ); // End of parallel_for

   Kokkos::fence ( "Fence position: loop on part" );
...
void push_momentum(std::vector<Particles> &particles, double dt) {
  ...
  for (std::size_t is = 0; is < particles.size(); is++) {
   ...
   Kokkos::fence ( "Fence momentum: loop on part" );
...
void project(const Params &params, ElectroMagn &em,
  ...
  for (std::size_t is = 0; is < particles.size(); is++) {
    ...
    const std::size_t n_particles = particle.size();
    const double inv_cell_volume_x_q =
        params.inv_cell_volume * particle.charge_m;

    const double params_dt = params.dt, 
                 params_inv_dx = params.inv_dx,
                 params_inv_dy = params.inv_dy,
                 params_inv_dz = params.inv_dz;
    ...
    Particles::view_t P_my = particle.my_m;
    Particles::view_t P_mz = particle.mz_m;
    Particles::view_t P_weight = particle.weight_m;

    // Electro view atomic
    Kokkos::View< double ***, Kokkos::MemoryTraits< Kokkos::Atomic >> 
       E_Jx = em.Jx_m, E_Jy = em.Jy_m, E_Jz = em.Jz_m;
	...
	KOKKOS_LAMBDA (const int part) {
      ...
      const double charge_weight =
          inv_cell_volume_x_q * P_weight(part);

      const double gamma_inv =
          1 / std::sqrt(1 + (P_mx(part) * P_mx(part) +
                             P_my(part) * P_my(part) +
                             P_mz(part) * P_mz(part)));

      const double vx = P_mx(part) * gamma_inv;
      const double vy = P_my(part) * gamma_inv;
      const double vz = P_mz(part) * gamma_inv;
      ...
      const double posxn =
          (Px(part) - 0.5 * params_dt * vx) * params_inv_dx +
          1;
      const double posyn =
          (Py(part) - 0.5 * params_dt * vy) * params_inv_dy +
          1;
      const double poszn =
          (Pz(part) - 0.5 * params_dt * vz) * params_inv_dz +
          1;
...
void solve_maxwell(const Params &params, ElectroMagn &em) {
  ...
  // Electric field Ex (d,p,p)
  Kokkos::parallel_for( mdrange_policy({0, 0, 0}, {Ex.extent(0), Ex.extent(1), Ex.extent(2)}),
    KOKKOS_LAMBDA(const int ix, const int iy, const int iz) {
  ...
  Kokkos::parallel_for( mdrange_policy({0, 0, 0}, {Ey.extent(0), Ey.extent(1), Ey.extent(2)}),
    KOKKOS_LAMBDA(const int ix, const int iy, const int iz) {
  ...
  Kokkos::parallel_for( mdrange_policy({0, 0, 0}, {Ez.extent(0), Ez.extent(1), Ez.extent(2)}),
    KOKKOS_LAMBDA(const int ix, const int iy, const int iz) {
  ...
  Kokkos::fence("Fence Electric fields");
 
  /////     Solve Maxwell Faraday (B)

  // Magnetic field Bx (p,d,d)
  Kokkos::parallel_for( mdrange_policy({0, 1, 1}, {Bx.extent(0), Bx.extent(1), Bx.extent(2)}),
    KOKKOS_LAMBDA(const int ix, const int iy, const int iz) {
  ...
  Kokkos::parallel_for( mdrange_policy({1, 0, 1}, {By.extent(0), By.extent(1), By.extent(2)}),
    KOKKOS_LAMBDA(const int ix, const int iy, const int iz) {
  ...
  Kokkos::parallel_for( mdrange_policy({1, 1, 0}, {Bz.extent(0), Bz.extent(1), Bz.extent(2)}),
    KOKKOS_LAMBDA(const int ix, const int iy, const int iz) {
  ...
  Kokkos::fence("Fence Magnetic fields");
...
void antenna(const Params &params, ElectroMagn &em,
  ...
  const double J_dual_zy_m = em.J_dual_zy_m,
               J_dual_zz_m = em.J_dual_zz_m;

  const double params_dy = params.dy,
               params_dz = params.dz,
               params_inf_y = params.inf_y,
               params_inf_z = params.inf_z;
  ...
  typedef Kokkos::MDRangePolicy<Kokkos::DefaultExecutionSpace, Kokkos::Rank<2>>
    ...
    KOKKOS_FUNCTION
    double fct_profile (double y, double z, double t) {
       return profile(y, z, t);
    };

    Kokkos::parallel_for(
        mdrange_policy({0, 0}, {Jz.extent(1), Jz.extent(2)}),
        KOKKOS_LAMBDA(const int iy, const int iz) {
  
      const double y =
          (iy - J_dual_zy_m * 0.5) * params_dy + params_inf_y - yfs;
      const double z =
          (iz - J_dual_zz_m * 0.5) * params_dz + params_inf_z - zfs;

      Jz(ix, iy, iz) = fct_profile(y, z, t);
    }
  );
  
  Kokkos::fence("fence antenna");
