...
void push_momentum(std::vector<Particles> &particles, double dt) {
  ...
  for (std::size_t is = 0; is < particles.size(); is++) {
    ...
    Kokkos::parallel_for("Initialize", n_particles, KOKKOS_LAMBDA(const int ip) {
    //for (std::size_t ip = 0; ip < n_particles; ++ip) {
      // 1/2 E
      double px = qp * Ex_m_copy(ip);
      double py = qp * Ey_m_copy(ip);
...
void project(const Params &params, ElectroMagn &em,
  ...
  for (std::size_t is = 0; is < particles.size(); is++) {
    ...
    Particles::view_t x_m_copy = particles[is].x_m;
    Particles::view_t y_m_copy = particles[is].y_m;
    Particles::view_t z_m_copy = particles[is].z_m;
